...
predicted<- function(qry){
  ...
  if(word_count(qry) >= 5 && any(with(sgm.df, grepl(paste(word(qry,-5:-1),collapse = " "), sgm.df$Gram_12345)))){ #Go for Six Gram with last 5 words
                    ...
                    #Order by Frequency of Gram_3 & return top 20
                    freq<-head(freq[order(-freq$Freq),],20)
                    predict<-as.character(freq[(freq$Freq>0),]$Var1)
                    predict<-as.character(na.omit(ifelse(removeWords(predict,word(qry,-5:-1))=="",NA,predict)))
                    predict<-removeWords(predict,"i")
                     return(predict)
    ...
    freq<-head(freq[order(-freq$Freq),],20)
    predict<-as.character(freq[(freq$Freq>0),]$Var1)
    predict<-as.character(na.omit(ifelse(removeWords(predict,word(qry,-4:-1))=="",NA,predict)))
    predict<-removeWords(predict,"i")
     return(predict)
    ...
    freq<-head(freq[order(-freq$Freq),],20)
    predict<-as.character(freq[(freq$Freq>0),]$Var1)
    predict<-as.character(na.omit(ifelse(removeWords(predict,word(qry,-3:-1))=="",NA,predict)))
    predict<-removeWords(predict,"i")
    return(predict)
    ...
    freq<-head(freq[order(-freq$Freq),],20)
    predict<-as.character(freq[(freq$Freq>0),]$Var1)
    predict<-as.character(na.omit(ifelse(removeWords(predict,word(qry,-2:-1))=="",NA,predict)))
    predict<-removeWords(predict,"i")
    return(predict)
    ...
    freq<-head(freq[order(-freq$Freq),],20)
    predict<-as.character(freq[(freq$Freq>0),]$Var1)
    predict<-as.character(na.omit(ifelse(removeWords(predict,word(qry,-1))=="",NA,predict)))
    predict<-removeWords(predict,"i")
    return(predict)
