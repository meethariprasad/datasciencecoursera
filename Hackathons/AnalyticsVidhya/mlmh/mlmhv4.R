...
#Update: Anothe way to looking at this is to get optimal set of variables 
# Which needs Imputation through RF and then you can do 
#Variable Importance again with Main Deep Learning Model. Howzthat@!
...
training.imp<-rbind(combi.LSI.Train,combi.LSI.Test)
str(training.imp)
training.imp$Customer_Since_Months<-as.factor(training.imp$Customer_Since_Months)
...
testing.imp<-rbind(combi.LSI.Train,combi.LSI.Test)
summary(testing.imp)
testing.imp$Customer_Since_Months<-as.factor(testing.imp$Customer_Since_Months)
...
h2o.varimp(final_dl_model)$variable
#Final train model error is 31% with Hit Ratio 68.7, 92,100

#How about rebuiding the model with train data feature engineering?
#First let us find anamolies and see what to do with them.
aut_encode_dl_model<- h2o.deeplearning(
  x=best_params$x, 
  training_frame=training.h2o,
  hidden = c(10, 10), epochs = 5, 
  autoencoder = TRUE
  ,adaptive_rate=best_params$adaptive_rate
  ) 

errors<-h2o.anomaly(aut_encode_dl_model,training.h2o,per_feature = FALSE)
er<-as.data.frame(errors)
plot(sort(er$Reconstruction.MSE), main='Reconstruction Error')
#Row observations with high mean squared error are anamolies
#http://stackoverflow.com/questions/35036498/how-to-interpret-the-reconstruction-mse-from-h2o-anomaly-detection
#From 0.35 it looks like the autoencoder is struggling.
training.h2o.imp<-training.h2o[which(er$Reconstruction.MSE < 0.35),]
#Running the model against train again.
#Should we do best parameters search again? Let see later.

split <- h2o.splitFrame( training.h2o.imp, c(0.6, 0.2))
train.hp <- h2o.assign( split[[1]], "train" ) # 60%
valid.hp <- h2o.assign( split[[2]], "valid" ) # 20%
test.hp  <- h2o.assign( split[[3]], "test" )  # 20%


##Best Parameter Calculation: Again
#Preserving Old Parameter and repeating all steps.
old.param<-best_params
hyper_params <- list(
  activation=c("Rectifier","Tanh","Maxout","RectifierWithDropout","TanhWithDropout","MaxoutWithDropout"),
  hidden=list(c(20,20),c(50,50),c(30,30,30),c(25,25,25,25)),
  input_dropout_ratio=c(0,0.05),
...
final_dl_model.imp<- h2o.deeplearning(
  x=best_params$x, y=best_params$y, 
  training_frame=training.h2o.imp, 
  model_id = NULL, 
  validation_frame = NULL, 
  nfolds = best_params$nfolds, 
  keep_cross_validation_predictions = best_params$keep_cross_validation_predictions, 
...
h2o.performance (final_dl_model.imp)
#The new hit ratio has been 69,92,1 with 30.77 Error%, Not bad.
#What is pending? The Variable Reduction using
#Variable Importance of final DL model and reconstructing model again?
h2o.varimp (final_dl_model.imp)$variable
#Around 36 Variables considered and 32 of them deemed important.
# We can keep only the columns and factors of those columns given in varimp
# and reconstruct our test and training set for prediction.
#Remember that this is not expected to overfit as we are having
#l1 and l2 parameters to reduce overfit or bias effect.
# [1] "Customer_Rating"                        
# [2] "Life_Style_Index"                       
# [3] "Destination_Type.1"                     
# [4] "Destination_Type.14"                    
# [5] "Confidence_Life_Style_Index.0.5"        
# [6] "Var2"                                   
# [7] "Destination_Type.5"                     
# [8] "Type_of_Cab.1"                          
# [9] "Destination_Type.10"                    
# [10] "Destination_Type.12"                    
# [11] "Destination_Type.2"                     
# [12] "Type_of_Cab.5"                          
# [13] "Destination_Type.4"                     
# [14] "Confidence_Life_Style_Index.3"          
# [15] "Destination_Type.9"                     
# [16] "Var3"                                   
# [17] "Var1"                                   
# [18] "Cancellation_Last_1Month"               
# [19] "Destination_Type.6"                     
# [20] "Gender.1"                               
# [21] "Type_of_Cab.3"                          
# [22] "Destination_Type.7"                     
# [23] "Destination_Type.11"                    
# [24] "Type_of_Cab.2"                          
# [25] "Confidence_Life_Style_Index.1"          
# [26] "Destination_Type.13"                    
# [27] "Type_of_Cab.4"                          
# [28] "Destination_Type.3"                     
# [29] "Destination_Type.8"                     
# [30] "Confidence_Life_Style_Index.2"          
# [31] "Trip_Distance"                          
# [32] "Gender.2" 
#All Selected Unique Variables.
# Customer_Rating
# Life_Style_Index
# Destination_Type
# Confidence_Life_Style_Index
...
h2o.str(testing.h2o)
submit.predict.SPT <- as.data.frame(h2o.predict(final_dl_model.imp, testing.h2o[,1:12]))
