...
# #Step2:Model Training and Cross Validation========
            ...
            vip.imp<-vip[which(vip$percentage*100>10),]$variable
            ...
            #RMSE on Test: 2461
            h2o.performance(dlmodel1,newdata = test)
...
# #Step3:Ensemble using Approximate Weighted Average (Or NN with 1 hiddenlayer?)========
            ...
            submit.predict.bestdl1$predict<-ifelse(submit.predict.bestdl1$predict<0,12,submit.predict.bestdl1$predict)
            summary(submit.predict.bestdl1$predict)
            
            submit.predict.bestdl2 <- as.data.frame(h2o.predict(gbm2, testing.h2o[vip.imp]))
            #If puchase is less than 0 then minimum puchase of training set 12.
            submit.predict.bestdl2$predict<-ifelse(submit.predict.bestdl2$predict<0,12,submit.predict.bestdl2$predict)
            summary(submit.predict.bestdl2$predict)
            
            submit.predict.bestdl3 <- as.data.frame(h2o.predict(dlmodel1, testing.h2o[vip.imp]))
            #If puchase is less than 0 then minimum puchase of training set 12.
            submit.predict.bestdl3$predict<-ifelse(submit.predict.bestdl3$predict<0,12,submit.predict.bestdl3$predict)
            summary(submit.predict.bestdl3$predict)
            
            submit.predict.bestdl<-NULL
            #submit.predict.bestdl$predict<-submit.predict.bestdl2$predict
            submit.predict.bestdl$predict<-(0.5*submit.predict.bestdl2$predict
                                          +0.5*submit.predict.bestdl3$predict)
...
# #Step4:Submission========
            ...
            write.csv(predict.bestdl,"bfv4.csv",row.names = F)
            #Private LB Score:2414. Top 2
# #Step5: Rework, if any=======
# 
